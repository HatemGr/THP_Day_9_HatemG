# Recuperation de l'adresse locale du fichier
local_dir = File.dirname(__FILE__)
$LOAD_PATH.unshift(local_dir)
require "00_j_methods.rb"
...
loop do
  ...
  when "1"
    #---- 1 - Nombre de handle
    list_length(journalists)
    break
  when "2"
    #---- 2 -  Handle le plus court
    list_shorter(journalists)
    break
  when "3"
    #---- 3 -  Nombre de handles de longueur 5
    handle_length_5(journalists)
    break
  when "4"
    #---- 4 -  Nombre de handles commencant par une majuscule
    start_maj(journalists)
    break
  when "5"
    #---- 5 -  By alphabetical order
    alpha_sort(journalists)
    break
  when "6"
    #---- 6 -  By size
    size_sort(journalists)
    break
  when "7"
    #---- 7 - Position de @epenser dans la liste
    epenser_pos(journalists)
    break
  when "8"
    #---- 8 -  Distribution des tailles de handles
    length_distr(journalists)
    break
  when "quit"
    break
  else
    puts "Saisie incorrect. Saisir un nombre entre 1 et 8 ou tapez quit: "
    next
  end
end
